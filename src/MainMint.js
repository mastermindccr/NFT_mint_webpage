...
const MainMint = ({ accounts, setAccounts }) => {
  ...
  const getNFTTotalSupply = async () => {
    ...
    const contract = new ethers.Contract(kryptoCampNFTAddress, kryptoCampNFTAbi, provider);
    const totalSupply = await contract.totalSupply();
    setTotalSupply(ethers.utils.formatUnits(totalSupply, 0));
  }
  ...
  const handleMint = async () => {
    if (window.ethereum) {
      ...
      const contract = new ethers.Contract(kryptoCampNFTAddress, kryptoCampNFTAbi, signer)

      try {
          // TODO: 4) 呼叫合約 mint 方法
          const mintPrice = 0.01;
          await contract.mint(mintAmount, {value: ethers.utils.parseEther((mintPrice * mintAmount).toString(), 'ether')});
      } catch ({ error }) {
